...
app.get('/api/debug-order-13', async (req, res) => {
  try {
    const [history] = await db.query("SELECT * FROM historial WHERE description LIKE '%740990%' OR description LIKE '%Jose Luis%' OR description LIKE '%orden 13%' OR description LIKE '%ID 13%' OR description LIKE '%ID #13%' ORDER BY id DESC");
    res.json({ history });
  } catch(e) {
