...
app.get('/api/admin/cleanup-pruebas', async (req, res) => {
  try {
    // 1. Get test maquilero IDs
    const [maquileros] = await db.query("SELECT id FROM maquileros WHERE nombre LIKE '%pruebas%'");
    const maquileroIds = maquileros.map(m => m.id);

    // 2. Get test inventario IDs
    const [inventarios] = await db.query("SELECT id FROM inventario WHERE modelo LIKE '%pruebas%'");
    const inventarioIds = inventarios.map(i => i.id);

    // Get all production IDs related to these
    let produccionIds = [];
    if (maquileroIds.length > 0 || inventarioIds.length > 0) {
      let prodQuery = "SELECT id FROM produccion WHERE 1=0";
      if (maquileroIds.length > 0) prodQuery += ` OR maquilero_id IN (${maquileroIds.join(',')})`;
      if (inventarioIds.length > 0) prodQuery += ` OR inventario_id IN (${inventarioIds.join(',')})`;
      const [producciones] = await db.query(prodQuery);
      produccionIds = producciones.map(p => p.id);
    }

    let deletedDiscounts = 0;
    let deletedPayments = 0;
    let deletedProductions = 0;
    let deletedRealStock = 0;
    let deletedCuts = 0;
    let deletedMaquileros = 0;
    let deletedHistory = 0;

    // Delete personal discounts
    if (maquileroIds.length > 0 || inventarioIds.length > 0) {
      let descQuery = "DELETE FROM descuentos_personales WHERE 1=0";
      if (maquileroIds.length > 0) descQuery += ` OR maquilero_id IN (${maquileroIds.join(',')})`;
      if (inventarioIds.length > 0) descQuery += ` OR inventario_id IN (${inventarioIds.join(',')})`;
      const [resDesc] = await db.query(descQuery);
      deletedDiscounts = resDesc.affectedRows;
    }

    // Delete pagos
    if (produccionIds.length > 0) {
      const [resPagos] = await db.query(`DELETE FROM pagos WHERE produccion_id IN (${produccionIds.join(',')})`);
      deletedPayments = resPagos.affectedRows;
    }

    // Delete produccion
    if (produccionIds.length > 0) {
      const [resProd] = await db.query(`DELETE FROM produccion WHERE id IN (${produccionIds.join(',')})`);
      deletedProductions = resProd.affectedRows;
    }

    // Delete inventario_real
    const [resInvReal] = await db.query("DELETE FROM inventario_real WHERE modelo LIKE '%pruebas%'");
    deletedRealStock = resInvReal.affectedRows;

    // Delete inventario
    if (inventarioIds.length > 0) {
      const [resInv] = await db.query(`DELETE FROM inventario WHERE id IN (${inventarioIds.join(',')})`);
      deletedCuts = resInv.affectedRows;
    }

    // Delete maquileros
    if (maquileroIds.length > 0) {
      const [resMaq] = await db.query(`DELETE FROM maquileros WHERE id IN (${maquileroIds.join(',')})`);
      deletedMaquileros = resMaq.affectedRows;
    }

    // Delete history
    const [resHist] = await db.query("DELETE FROM historial WHERE description LIKE '%pruebas%' OR target LIKE '%pruebas%'");
    deletedHistory = resHist.affectedRows;

    res.json({
      success: true,
      message: "Limpieza de datos de prueba ('pruebas') completada exitosamente.",
      details: {
        deletedDiscounts,
        deletedPayments,
        deletedProductions,
    ...
    console.error("Error en cleanup-pruebas:", error);
    res.status(500).json({ error: error.message });
  }
});
